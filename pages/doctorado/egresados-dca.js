document.addEventListener("DOMContentLoaded", function () {
  ...
  const searchInput = document.getElementById("busquedaEgresadoDCA");
  const cohorteFilter = document.getElementById("filtroCohorteEgresadoDCA");
  const tbody = document.querySelector("#tablaEgresadosDCA tbody");
  const paginationUl = document.getElementById("paginacionEgresadosDCA");

  const limit = 10;
  let currentPage = 1;
  let filteredData = [...DATA_EGRESADOS_DCA];

  // Poblar selector de generación
  [...new Set(DATA_EGRESADOS_DCA.map(d => d.cohorte))].sort().reverse().forEach(c => {
  ...
  function applyFilter() {
    ...
    filteredData = DATA_EGRESADOS_DCA.filter(d => 
