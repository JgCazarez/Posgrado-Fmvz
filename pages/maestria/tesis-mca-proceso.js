document.addEventListener("DOMContentLoaded", function () {
  ...
  const searchInput = document.getElementById("busquedaAlumnoMCA");
  const cohorteFilter = document.getElementById("filtroCohorteMCA");
  const tbody = document.querySelector("#tablaTesisMCA tbody");
  const paginationUl = document.getElementById("paginacionMCA");

  const limit = 10;
  let currentPage = 1;
  let filteredData = [...DATA_MCA];

  // Poblar selector de cohorte
  [...new Set(DATA_MCA.map(d => d.cohorte))].sort().reverse().forEach(c => {
  ...
  function applyFilter() {
    ...
    filteredData = DATA_MCA.filter(d => 
